const legal = {
  cgv: {
    ...
    sections: [
      ...
      { heading: 'Services', paragraphs: ["Les services proposés sont décrits sur le site (dépannage et réparation, montage PC, récupération de données, conseil, forfaits, réseau, création de sites web vitrines ou dynamiques avec base de données, option espace membre avec authentification et données de profil, etc.). Le détail et le périmètre de chaque prestation sont fixés dans le devis ou la proposition remis au client. La création de sites dynamiques ou avec espace membre implique le traitement de données personnelles (ex. : email, téléphone) sur le site livré ; les modalités et responsabilités (responsable du traitement, information des utilisateurs, conformité RGPD) sont précisées au devis et dans les documents contractuels. La prestation ne comprend pas la mise en place de plateformes de vente en ligne ni la gestion de données de paiement (paiement en ligne)."] },
      ...
      { heading: 'Déplacement', paragraphs: ["Le déplacement est gratuit dans un rayon de 15 km autour d'Ahuillé. Au-delà, il est facturé 0,50 € par kilomètre. Le détail est rappelé sur le devis."] },
  ...
  cgu: {
    ...
    sections: [
      { heading: 'Objet', paragraphs: ["Le site présente l'activité d'Arkenyx (dépannage et réparation, montage PC, récupération de données, conseil, forfaits, réseau, création de sites web vitrines et dynamiques, etc.) et permet de prendre contact par e-mail, téléphone, formulaire de contact ou demande de devis en ligne."] },
  ...
  confidentialite: {
    ...
    sections: [
      ...
      { heading: 'Sites livrés avec base de données ou espace membre', paragraphs: ["Lorsqu'Arkenyx réalise pour un client un site web doté d'une base de données ou d'un espace membre (authentification, profils utilisateur, email, téléphone), le site ainsi livré est la propriété du client. Pour les données collectées et traitées via ce site (formulaires, comptes utilisateurs, etc.), le client est responsable du traitement au sens du RGPD. Arkenyx met en œuvre des mesures techniques et organisationnelles adaptées lors du développement (sécurisation des accès, bonnes pratiques) et peut préciser au devis les recommandations (politique de confidentialité, information des utilisateurs). La gestion des données personnelles sur le site livré relève du client après mise en ligne ; Arkenyx ne gère pas les données de paiement ni les plateformes de vente en ligne."] },
